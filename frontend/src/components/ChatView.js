    ...
    <Box sx={{ flexGrow: 1, overflowY: 'auto', p: 2, display: 'flex', flexDirection: 'column' }}>
      {messages.map((msg, index) => {
        ...
        return (
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'flex-start', flexDirection: isUserAuditor ? 'row-reverse' : 'row' }}>
              {!isUserAuditor && (
                <Avatar sx={{ bgcolor: getAvatarColor(senderDisplayName), mr: isUserAuditor ? 0 : 1, ml: isUserAuditor ? 1 : 0}}>
                  {avatarChar}
                </Avatar>
              )}
              <Paper 
                ...
                sx={{
                  p: 1.5,
                  bgcolor: isUserAuditor ? 'primary.light' : 'background.paper',
                  color: isUserAuditor ? 'primary.contrastText' : 'text.primary',
                  borderRadius: isUserAuditor ? '20px 20px 5px 20px' : '20px 20px 20px 5px',
                  maxWidth: '70%',
                  wordWrap: 'break-word',
                }}
              >
                ...
                <Box sx={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 0.5}}>
                    {!isUserAuditor && 
                        <Chip label={senderDisplayName} size="small" sx={{mr:1, backgroundColor: getAvatarColor(senderDisplayName), color: 'white'}}/>
                    }
                    <Typography variant="caption" color={isUserAuditor ? 'rgba(255,255,255,0.7)' : 'text.secondary'} sx={{ml: isUserAuditor ? 0: 'auto'}}>
                        {messageDate}
                    </Typography>
                </Box>
              </Paper>
